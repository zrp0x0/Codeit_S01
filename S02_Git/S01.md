...
### git reset --hard 복구 방법
- git reflog: 현재 왓다 갔다한 모든 commit 기록을 보여줌
    - git log: 현재 유효한 commit 기록을 보여줌
- 모든 커밋 기록 및 아이디를 보여줌
- git reset --hard <복구할 해당 커밋 아이디>

### git restore vs git reset
- git restore --staged는 최신 Git 버전(2.23+)에서 권장되는 명령어
- 예전 자료에는 git reset HEAD <file>로 되어 있는 경우가 많으나, 두 개가 같은 역할

### 실수로 github에 gitignore안하고 올렸다면?
```bash
git rm -r --cached .  # 캐시 삭제 (실제 파일은 안 지워짐)
git add .             # 다시 stage (gitignore 적용됨)
